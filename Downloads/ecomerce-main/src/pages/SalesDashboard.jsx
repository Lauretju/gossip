...
const SalesDashboard = () => {
  const { sales, loading, getSales, getAllSales, updateSaleStatus } = useSales();
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Estado actualizado a "${getPaymentStatusText(newStatus)}" correctamente`,
        severity: 'success'
      });
      
      handleCloseDialog();
      loadSales(); // Recargar las ventas para reflejar el cambio
    } catch (error) {
    ...
    >
      ...
      >
        <DialogTitle sx={{ 
          ...
          ✏️ Editar Estado de Pago
        </DialogTitle>
        <DialogContent sx={{ p: 3 }}>
          {selectedSale && (
            <Box>
              <Typography variant="body1" sx={{ color: '#ff6b9d', mb: 2 }}>
                <strong>Venta ID:</strong> {isMobile ? `${selectedSale.id.substring(0, 12)}...` : selectedSale.id}
              </Typography>
              <Typography variant="body1" sx={{ color: '#ff6b9d', mb: 2 }}>
                <strong>Cliente:</strong> {getCustomerName(selectedSale)}
              </Typography>
              <Typography variant="body1" sx={{ color: '#ff6b9d', mb: 3 }}>
                <strong>Total:</strong> ${selectedSale.total?.toLocaleString() || 0}
              </Typography>
              
              <FormControl fullWidth>
                <InputLabel sx={{ color: '#ff6b9d' }}>Nuevo Estado</InputLabel>
                <Select
                  value={newStatus}
                  label="Nuevo Estado"
